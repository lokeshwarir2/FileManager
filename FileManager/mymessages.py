...
class Ui_otherfiles(object):
    def __init__(self):
        ...
        user=u.splitlines()
        path = os.path.abspath("/home/" + user[0] + "/messages")
        command1= "ls "+path
        f = (os.popen(command1).read())
        self.files1=f.splitlines()
        super().__init__()
    def setupUi(self, otherfiles):
        ...
        display_file(self,self.files1)

    def retranslateUi(self, otherfiles):
        _translate = QtCore.QCoreApplication.translate
        otherfiles.setWindowTitle(_translate("otherfiles", "MY MESSAGES >> FILE MANAGER"))
        self.label.setText(_translate("otherfiles", "MY MESSAGES"))
        self.pushButton.setText(_translate("otherfiles", "No messages sent"))
        self.pushButton_2.setText(_translate("otherfiles", "No messages sent"))
        self.pushButton_3.setText(_translate("otherfiles", "No messages sent"))
        self.pushButton_4.setText(_translate("otherfiles", "No messages sent"))
        self.pushButton_5.setText(_translate("otherfiles", "No messages sent"))
        self.pushButton_6.setText(_translate("otherfiles", "No messages sent"))
    ...
    def openWindow3(self,pushbutton):
        ...
        if(file!="No messages sent"):
            u = os.popen("whoami").read()
            user = u.splitlines()
            filename = "/home/"+user[0]+"/messages/" + file
            command = "geany '" + filename + "'"
            os.system(command)

    def openWindow(self, pushbutton):
        delete = pushbutton.text()
        if (delete != "No messages sent"):
            u = os.popen("whoami").read()
            user = u.splitlines()
            command = "sudo rm  '/home/" + user[0] + "/messages/" + delete + "'"
            os.system(command)
